...
static int positionComputationWRTPermutation(int ntx, int nrx, int nltf, int num_tones, int nrxIdx, int nltfIdx, int ntxIdx, int subcarrierIdx, const struct ExtraInfo & rxExtraInfo) {
    ...
    return ntxIdx * (nrx * nltf * num_tones) + nltfIdx * (nrx * num_tones) + new_nrxIdx * num_tones + subcarrierIdx;
}
        ...
        for (auto nrxIdx = 0 ; nrxIdx < nrx ; nrxIdx ++ )
            for (auto nltfIdx = 0; nltfIdx < nltf; nltfIdx ++)
                for(auto ntxIdx = 0 ; ntxIdx < ntx ; ntxIdx ++ ) {
                    auto position = positionComputationWRTPermutation(ntx, nrx, nltf, num_tones, nrxIdx, nltfIdx, ntxIdx, subcarrierIdx, rxExtraInfo);
